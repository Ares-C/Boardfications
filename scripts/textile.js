...
function prep(m){
	...
	m = make_image(m,/!([^!\s]+)!:(\S+)/);
	m = make_image(m,/!([^!\s]+)!/);
	m=m.replace(/{{sticker:(.*?)}}/g, function (match, capture) {
		if (capture.search("sg-") != -1) {
			endpoint = "starguardian"
		} else {
			endpoint = "slayer140"
		}
		return '<span class="sticker slayer" style="background-image: url(https://cdn.leagueoflegends.com/stickers/'+endpoint+'/'+capture+'.png); background-size: contain;"></span>';
	});
	m=m.replace(/{{item:(.*?)}}/g, '<span class="item" style="background: url(http://ddragon.leagueoflegends.com/cdn/'+gameversions['item']+'/img/item/$1.png); background-size: contain"></span>')
	m=m.replace(/{{champion:(.*?)}}/g, function (match, capture) {
		return '<span class="item" style="background: url(http://ddragon.leagueoflegends.com/cdn/'+gameversions['sticker']+'/img/champion/'+champion_name_by_id[parseInt(capture)]+'.png); background-size: contain"></span>'
	});
	m=m.replace(/{{summoner:(.*?)}}/g, function (match, capture) {
		return '<span class="item" style="background: url(http://ddragon.leagueoflegends.com/cdn/'+gameversions['summoner']+'/img/spell/'+summoner_spell_by_id[parseInt(capture)]+'.png); background-size: contain"></span>'
	});
...
function parse_quotes(m) {
	if (m.search("[{quoted}]") != -1) {
		console.log("match")

		name = m.match(/name=(.*?),realm/g)
		realm = m.match(/realm=(.*?),/g)
		timestamp = new Date(m.match(/timestamp=(.*?)\)/g))
		// if (timestamp) {
		// 	timestamp = timestamp.getTime()
		// }
		elapsed = moment(timestamp).fromNow()

		console.log(Date.now(), timestamp, Date.now()-timestamp, elapsed)
		m.replace(/> \[{quoted}].*/, '<span class="markdown-meta markdown-meta-quoted"><span>'+name+' ('+realm+')</span> - <span class="timeago">'+elapsed+'</span></span>') // clear quote info
	}
	return m;
}
